...
import { Address, SystemProgram, Transaction, TransactionMessage, VersionedTransaction } from '@solana/web3.js';
import { act, renderHook } from '@testing-library/react';
import React from 'react';
import type { ReactNode } from 'react';
import { ConnectionProvider } from '../web3js/ConnectionProvider.js';
import { useAnchorWallet } from '../web3js/useAnchorWallet.js';
import { useConnection } from '../web3js/useConnection.js';
import { useWallet } from '../web3js/useWallet.js';
...
function makeModifyingSigner(address: string) {
    return {
        address,
        modifyAndSignTransactions: jest.fn(async (transactions: readonly KitTransaction[]) =>
...
function makeLegacyTransaction(payer: Address) {
...
function makeVersionedTransaction(payer: Address) {
...
describe('useWallet (web3js)', () => {
    it('exposes the connected address as a web3.js Address', () => {
        ...
        expect(result.current.publicKey).toBeInstanceOf(Address);
    ...
    it('exposes no signTransaction for sign-and-send-only wallets', () => {
        const signer = { address: PAYER_ADDRESS, signAndSendTransactions: jest.fn() };
        const { client } = makeConnectedClient(signer);
        const { result } = renderHook(() => useWallet(), { wrapper: makeWrapper(client) });
    ...
    it('sends through the wallet when it can sign and send in one step', async () => {
        const signer = {
            ...
            signAndSendTransactions: jest.fn(async () => [SIGNATURE]),
        };
        const { client } = makeConnectedClient(signer);
        const { result } = renderHook(() => useWallet(), { wrapper: makeWrapper(client) });
        const connection = { sendRawTransaction: jest.fn() } as unknown as Connection;
    ...
    it('signs and sends through the connection otherwise', async () => {
        ...
        const { client } = makeConnectedClient(signer);
        const { result } = renderHook(() => useWallet(), { wrapper: makeWrapper(client) });
        const sendRawTransaction = jest.fn(async () => 'mock-signature');
