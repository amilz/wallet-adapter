...
import { useWallet as useKitWallet } from '../useWallet.js';
import {
    canSignTransactions,
...
export type WalletContextState = UseWalletResult & {
    /** The connected account's address as a web3.js `Address` (aka `PublicKey`), or `null`. */
    publicKey: Address | null;
    /** Connect to the discovered wallet with this name, or disconnect when passed `null`. */
    select(walletName: string | null): void;
    /**
     * Sign the transaction with the connected wallet and broadcast it. Wallets that support
     * `solana:signAndSendTransaction` broadcast it themselves; otherwise the signed transaction is
     * sent through `connection`. Resolves with the transaction signature.
     */
    sendTransaction(
    ...
    /**
     * Sign transactions with the connected wallet, resolving with signed transactions of the same
     * class. `undefined` when disconnected or when the wallet can only sign-and-send.
     */
    signAllTransactions:
...
/**
 * The Kit-native {@link import('../useWallet.js').useWallet | useWallet} extended with the
 * `@solana/web3.js` surface of the classic `@solana/wallet-adapter-react` hook: `publicKey`,
 * `select`, `signTransaction`, `signAllTransactions`, and `sendTransaction`.
 *
 * Use it with {@link ConnectionProvider} as a near drop-in replacement when migrating a
 * wallet-adapter app from web3.js v1 to v3.
 */
export function useWallet(): WalletContextState {
    const kitWallet = useKitWallet();
    const { address, connect, disconnect, signer, wallets } = kitWallet;

    const publicKey = useMemo(() => (address ? new Address(address) : null), [address]);
