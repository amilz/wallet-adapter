...
/**
 * A single hook mirroring the shape of the classic `@solana/wallet-adapter-react` `useWallet()`,
 * implemented over the Kit wallet plugin.
 *
 * Values (`wallets`, `account`, `status`, …) come from the connected wallet's reactive store;
 * the action fields (`connect`, `disconnect`, `signMessage`, `signIn`) are stable callbacks. Use
 * the `*Async` variants when you need the resolved value or want to catch failures.
 */
